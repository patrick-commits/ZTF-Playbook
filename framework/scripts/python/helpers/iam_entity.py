...
class IamEntity(Entity):
    __BASEURL__ = "api/iam/authn/"
    version = "v1"
    resource_type = ""
    ...
    def __init__(self, session: RestAPIUtil, proxy_cluster_uuid=None):
        ...
        super(IamEntity, self).__init__(session=session, resource_type=resource_type)
    ...
    def read(self, **kwargs):
        ...
        return super(IamEntity, self).read(endpoint=endpoint, **kwargs)

    def create(self, **kwargs):
        endpoint = self.get_proxy_endpoint(kwargs.pop("endpoint", ""))
        return super(IamEntity, self).create(endpoint=endpoint, **kwargs)

    def update(self, **kwargs):
        endpoint = self.get_proxy_endpoint(kwargs.pop("endpoint", ""))
        return super(IamEntity, self).update(endpoint=endpoint, **kwargs)

    def delete(self, **kwargs):
        endpoint = self.get_proxy_endpoint(kwargs.pop("endpoint", ""))
        return super(IamEntity, self).delete(endpoint=endpoint, **kwargs)
